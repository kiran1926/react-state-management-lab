...
// add fighter to team
  ...
  const totalStrength = team.reduce((acc, fighter) => 
    acc + fighter.strength, 0);

  // total team agility
  const totalAgility = team.reduce((acc, fighter) => 
  acc + fighter.agility, 0);

  // remove fighter
  const removeFighter = (fighterToRemove) => {
  ...
  return (
    <>
      ...
      <div>
        <h3>Your Team</h3>
        {team.length === 0 ? ( <p>Pick some members! </p>) : (
          <ul>
            {team.map((fighter) => (
              <li key={fighter.id}>
      ...
      <h2>Available Fighters :  </h2>
        <ul>
          {zombieFighters.map(( fighter, id ) => (
            <li key={id}>
                <img src={fighter.img} alt={`image for ${fighter.name} fighter`} />
                <p>{fighter.name}</p>
                <p>Price: {fighter.price}</p>
                <p>Strength: {fighter.strength}</p>
                <p>Agility: {fighter.agility}</p>
